...
lsObj <- function (numls = 100, sizeB = 0, pretty = FALSE) {	
					...
					for (item in listLs){
						...
						if (is.environment(objitem)) {
							...
							tmpEnv <- sapply(listEnv, function(x){object.size(get(x,envir = objitem))})
#							for (tmpItem in listEnv) { # objects in user-defined ENV
#                 tmpEnv <- c(tmpEnv, object.size(get(tmpItem,envir = objitem)))
#							}
							nameEnv <- sapply(listEnv, function(x){names(x) <- paste(item,"$",x,sep = "")})
						}
						if (is.function(objitem)){
							listFun <- ls(environment(objitem))
